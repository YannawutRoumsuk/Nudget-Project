...
|-----|---------------|-----------|
| แยกข้อความที่กฎอ่านไม่ออก เช่น `จ่ายค่าข้าวไปห้าสิบบาท` | เฉพาะตอนกฎหายอดหรือหมวดไม่เจอ | `LLM_PROVIDER` / `LLM_MODEL` |
| บทวิเคราะห์รายเดือนในหน้า `วิเคราะห์` | เฉพาะตอนผู้ใช้กดปุ่ม แคชตามตัวเลข จำกัดวันละ 10 ครั้งต่อคน | `LLM_PROVIDER` / `LLM_MODEL` |
| อ่านสลิปจากรูป | ทุกสลิป ถ้าไม่มีคีย์จะถอยไปใช้ Tesseract ในเครื่อง | `GEMINI_API_KEY` / `OCR_PROVIDER` / `OCR_MODEL` |
