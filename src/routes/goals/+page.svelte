...
<section class="card create">
	...
	<form method="POST" action="?/create" class="fields">
		...
		<label>ประเภทเป้าหมาย<select name="goalType"><option value="planned" selected>เป้าหมาย/รายจ่ายก้อนใหญ่</option><option value="emergency">เงินสำรองฉุกเฉิน</option></select></label>
...
<section class="goals" aria-label="เป้าหมายของฉัน">
	...
	{#each data.goals as goal (goal.id)}
		...
		<article class="card goal" class:muted={goal.status !== 'active'}>
			<header><div><p class="eyebrow">{goal.goalType === 'emergency' ? 'เงินสำรองฉุกเฉิน' : 'เป้าหมาย/รายจ่ายก้อนใหญ่'} · {goal.status === 'active' ? 'กำลังดำเนินการ' : goal.status === 'paused' ? 'พักไว้' : goal.status === 'completed' ? 'ถึงเป้าหมายแล้ว' : 'ปิดแล้ว'} · สำคัญระดับ {goal.priority}</p><h2>{goal.name}</h2></div><strong class="amount">฿{formatNumber(goal.currentAmount)} <small>/ ฿{formatNumber(goal.targetAmount)}</small></strong></header>
