	...
	<section class="card changes">
		<div class="section-title"><div><h2>สมมติฐาน ({changes.length}/20)</h2><p>แต่ละข้อคำนวณร่วมกันในเดือน {data.month.key}</p></div>
			<div class="section-actions">
				...
				<form method="POST" action="?/simulate"><input type="hidden" name="changes" value="[]" /><button>ล้างสมมติฐาน</button></form>
			</div>
		</div>
...
<style>
	...
	.section-title,.measure div{display:flex;align-items:center;justify-content:space-between;gap:.8rem}.section-actions{display:flex;align-items:center;gap:.5rem}.changes ul{margin-top:.8rem}.changes li{display:flex;justify-content:space-between;align-items:center;gap:1rem;padding:.55rem 0;border-bottom:1px solid var(--rule)}
