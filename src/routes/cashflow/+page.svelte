			...
			{#if day.events.length}
				<div class="events">
					{#each day.events.slice(0, 4) as event, i (`${day.key}-${i}-${event.title}`)}
						<a class="event" class:forecast={event.status === 'forecast'} class:expense={event.type === 'expense'} class:income={event.type === 'income'} href={event.href ?? '#'} title={event.title}>
							<span>{event.title}</span><strong>{event.type === 'expense' ? '−' : '+'}{formatNumber(event.amount)}</strong>
						</a>
					{/each}
